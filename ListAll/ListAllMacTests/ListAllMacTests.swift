...
// MARK: - Consistent Empty State Components Tests (Task 12.7)

/// Tests for Task 12.7: Consistent Empty State Components
/// Verifies that empty states use the comprehensive MacItemsEmptyStateView
/// and that search empty states get dedicated messaging.
...
// MARK: - Quick Entry Window Tests (Task 12.10)

/// Tests for Task 12.10: Add Quick Entry Window
/// Verifies that QuickEntryView provides a fast way to add items from anywhere in macOS
/// Power users expect Things 3-style Quick Entry feature.
...
// MARK: - Archived Lists Tests (macOS Bug Fix)

/// TDD tests for the macOS archived lists bug fix.
///
/// BUG DESCRIPTION:
/// The macOS archived lists view shows empty because `MacSidebarView.displayedLists`
/// filters `dataManager.lists`, but `dataManager.lists` never contains archived lists
/// (loadData() excludes them with `isArchived == NO OR isArchived == nil` predicate).
///
/// FIX BEING IMPLEMENTED:
/// 1. Add `@Published var archivedLists: [List] = []` to DataManager
/// 2. Add `loadArchivedData()` method to populate the archivedLists property
/// 3. MacSidebarView will use `dataManager.archivedLists` when `showingArchivedLists == true`
/// 4. After `restoreList()` and `permanentlyDeleteList()`, refresh archivedLists
///
/// These tests are written FIRST per TDD methodology (red phase).
/// The actual implementation will be added after tests define the expected behavior.
