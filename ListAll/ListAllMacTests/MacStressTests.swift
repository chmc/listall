//
//  MacStressTests.swift
//  ListAllMacTests
//
//  Stress tests for volume operations and rapid state changes on macOS.
//  Mirrors iOS StressTests adapted for macOS TestHelpers (ListModel typealias, etc.).
//
...
final class MacStressTests: XCTestCase {
    ...
    func testValidateDataIntegrity_validData_returnsEmpty() {
        let listId = UUID()
        var list = ListModel(name: "Valid List")
        list.id = listId
    ...
    func testValidateDataIntegrity_largeVolume_noFalsePositives() {
        ...
        for i in 0..<100 {
            let listId = UUID()
            var list = ListModel(name: "List \(i)")
            list.id = listId
            var items: [Item] = []
            for j in 0..<10 {
    ...
    func testValidateListBusinessRules_duplicateName_detectsFailure() {
        var existingList = ListModel(name: "Groceries")
        existingList.id = UUID()
        existingList.orderNumber = 0

        var newList = ListModel(name: "groceries")  // Same name, different case
        newList.id = UUID()
    ...
    func testValidateListBusinessRules_duplicateOrderNumber_detectsFailure() {
        var existingList = ListModel(name: "List A")
        existingList.id = UUID()
        existingList.orderNumber = 5

        var newList = ListModel(name: "List B")  // Different name, same order number
        newList.id = UUID()
    ...
    func testValidateListBusinessRules_sameId_noFalsePositive() {
        let sharedId = UUID()
        var list = ListModel(name: "My List")
        list.id = sharedId
        list.orderNumber = 0
    ...
    func testValidateListBusinessRules_noExistingLists_returnsEmpty() {
        let newList = ListModel(name: "New List")
    ...
    func testValidateListBusinessRules_bothViolations_reportsAll() {
        ...
        var newList = ListModel(name: "GROCERIES")
        newList.id = UUID()
